...
export function PropostaItensExtras({ items, setItems, titulo, numPessoas }: PropostaSecaoProps) {
  ...
  // Função para obter todos os itens (incluindo subprodutos)
  const getAllItems = () => {
  ...
  const renderItemRow = (item: LinhaItem, isSubproduto = false) => {
    ...
    return (
      <tr key={item.id} className={`border-b last:border-0 ${backgroundColor}`}>
        <td className={`${paddingLeft} pr-3 py-1`}>
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <Input
                placeholder={isSubproduto ? "Clique para selecionar subproduto..." : "Clique para selecionar um item..."}
        ...
        <td className="px-3 py-1 text-right">
          {isSubproduto
            ? total(item).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })
