...
export function PropostaItensExtras({ items, setItems, titulo }: PropostaSecaoProps) {
  ...
  const addExtra = () => setItems([...items, { id: crypto.randomUUID(), produtoId: null, descricao: '', valorUnitario: 0, quantidade: 1, descontoPermitido: 0, descontoAplicado: 0, tipoItem: 'produto', calculoAutomatico: false }]);
  const removeExtra = (id: string) => setItems(items.filter(e => e.id !== id));
  ...
  const change = (id: string, field: keyof LinhaItem, value: string | number) => {
    setItems(items.map(e => {
      if (e.id !== id) return e;
      if (field === 'descricao') {
        return { ...e, descricao: String(value) };
      }
  ...
  return (
    <TooltipProvider>
        ...
        <table className="min-w-full text-sm border-collapse">
          <thead>
            <tr className="border-b bg-zinc-100 dark:bg-zinc-900/40">
              <th className="text-left px-3 py-2">Item Extra</th>
          ...
          <tbody>
            {items.map(item => (
              <tr key={item.id} className="border-b last:border-0">
                ...
                <td className="px-3 py-1 text-right">
                  <div className="flex items-center gap-1">
                    <Input 
                      ...
                      onChange={e => change(item.id, 'valorUnitario', Number(e.target.value))} 
                ...
                <td className="text-center">{items.length > 1 && <Button variant="ghost" size="icon" onClick={() => removeExtra(item.id)}><Trash2 className="h-4 w-4 text-red-500"/></Button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="p-2 flex justify-end border-t">
        <Button variant="outline" size="sm" onClick={addExtra}><Plus className="h-4 w-4 mr-1"/>Adicionar Item</Button>
      </div>
       <SelecaoProdutoModal
        open={isModalOpen}
        onOpenChange={setIsModalOpen}
        onSelect={handleProductSelect}
        seguimentoFiltro="itens_extra"
      />
      </div>
    </TooltipProvider>
  );
