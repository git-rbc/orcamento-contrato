...
import { CreateEspacoEventoDTO, EspacoEventoLayout } from '@/types/database';
...
export default function NovoEspacoEventoPage() {
  const router = useRouter();
  const { createEspaco } = useEspacosEventos();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.layouts.length > 0 && result.data?.id) {
        await fetch(`/api/espacos-eventos/${result.data.id}/layouts`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Novo Espaço para Eventos</h1>
          <p className="text-muted-foreground">
            Adicione um novo espaço/ambiente para eventos
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <LayoutsManager
          ...
          onChange={(layouts) => setFormData({ ...formData, layouts })}
